...
def generate_profile_id(username: str, max_length: int = 8) -> str:
    """
    Generate a unique profile ID from username using unique letters.

    Extracts unique alphanumeric characters from the username in order of first appearance,
    preserving the sequence and creating a memorable profile identifier.

    Args:
        username: The username to extract unique letters from
        max_length: Maximum length of the generated ID (default: 8)

    Returns:
        A unique profile ID consisting of uppercase letters and digits from the username
    """
    if not username:
        return "GUEST"

    # Extract only alphanumeric characters and keep unique ones in order
    unique_chars = []
    seen = set()
    ...
    # Take first max_length characters, or pad if needed
    profile_id = ''.join(unique_chars[:max_length])

    # If profile_id is shorter than 3 characters, pad with dashes or return as is
    if len(profile_id) < 3:
        profile_id = profile_id or "USER"
